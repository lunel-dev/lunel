...
try {
  ...
  TcpSocket = (tcpSocketModule?.default ??
    tcpSocketModule) as typeof import("react-native-tcp-socket").default;
  logger.info("proxy", "loaded react-native-tcp-socket module", {
    ...
    hasCreateServer: Boolean(
      (tcpSocketModule?.default ?? tcpSocketModule)?.createServer,
    ),
  });
} catch (error) {
...
function randomRetryJitterMs(): number {
  ...
  return (
    PROXY_WS_RETRY_JITTER_MIN_MS + Math.floor(Math.random() * (spread + 1))
  );
}
...
function extractHttpPreview(
  preview: string,
): { startLine: string; headers: Record<string, string> } | null {
  const headerEnd =
    preview.indexOf("\r\n\r\n") >= 0
      ? preview.indexOf("\r\n\r\n")
      : preview.indexOf("\n\n");
  ...
  const isHttpRequest =
    /^(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\s+\S+\s+HTTP\/\d/i.test(
      startLine,
    );
...
function classifyInitialClientTraffic(
  chunks: Uint8Array[],
): "insufficient_data_yet" | "unsupported" | InitialClientProtocol {
  ...
  if (bytes[0] === 0x16) {
    ...
    const looksLikeTlsVersion =
      major === 0x03 && minor >= 0x00 && minor <= 0x04;
  ...
  const preview = decodePreview(
    bytes.slice(0, Math.min(bytes.byteLength, 2048)),
  );
  ...
  if (
    /^(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\s+\S+\s+HTTP\/1\.[01]$/i.test(
      requestLine,
    )
  ) {
...
async function sniffInitialClientProtocol(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const finish = (
      result: {
        protocol: InitialClientProtocol;
    ...
    const handleData = (data: any) => {
      ...
      if (
        classification === "http_plaintext" ||
        classification === "tls_client_hello"
      ) {
      ...
      if (classification === "unsupported") {
        finish(
          null,
          Object.assign(new Error("Unsupported localhost client protocol"), {
    ...
    const handleEnd = () => {
      finish(
        ...
        Object.assign(
          new Error(
            "Localhost client closed before sending supported protocol bytes",
          ),
          { code: "ECONNRESET" },
        ),
      );
    ...
    const handleClose = () => {
      finish(
        null,
        Object.assign(
          new Error("Localhost client closed during protocol sniff"),
          { code: "ECONNRESET" },
        ),
      );
    };
    ...
    const handleError = () => {
      finish(
        null,
        Object.assign(
          new Error("Localhost client socket error during protocol sniff"),
          { code: "ECONNRESET" },
        ),
      );
    };
    ...
    const timeout = setTimeout(() => {
      finish(
        ...
        Object.assign(
          new Error(
            "Localhost client did not send supported protocol bytes in time",
          ),
          { code: "ETIMEOUT" },
        ),
      );
    ...
    try {
      ...
      finish(
        null,
        Object.assign(
          new Error("Failed to resume localhost socket for protocol sniff"),
          { code: "EIO" },
        ),
      );
    }
...
function sendBufferedClientChunks(
  proxyWs: WebSocket,
  chunks: Uint8Array[],
): void {
  for (const chunk of chunks) {
...
function maybeLogClientPayload(tunnel: TunnelInfo, chunk: Uint8Array): void {
  ...
  if (tunnel.clientPreview.length < 4096) {
    tunnel.clientPreview += decodePreview(chunk).slice(
      0,
      4096 - tunnel.clientPreview.length,
    );
  }
...
function maybeLogServerPayload(tunnel: TunnelInfo, chunk: Uint8Array): void {
  ...
  if (tunnel.serverPreview.length < 4096) {
    tunnel.serverPreview += decodePreview(chunk).slice(
      0,
      4096 - tunnel.serverPreview.length,
    );
  }
...
function startSingleServer(port: number): void {
  if (!TcpSocket) {
    logger.warn(
      "proxy",
      "tcp socket module unavailable; cannot bind localhost port",
      { port },
    );
    return;
  }
  ...
  server.on("listening", () => {
    ...
    logger.info("proxy", "localhost proxy server listening", {
      port,
      host: "127.0.0.1",
    });
  });
  ...
  server.on("error", (error: any) => {
    ...
    if (error?.code === "EADDRINUSE") {
      const recentlyStopped =
        Date.now() - (lastStoppedAt.get(port) || 0) < QUICK_RETRY_WINDOW_MS;
      const retryDelay = recentlyStopped
        ? QUICK_RETRY_DELAY_MS
        : BIND_RETRY_COOLDOWN_MS;
...
async function handleIncomingConnection(
  ...
  try {
    ...
    logger.info(
      "proxy",
      "dropping localhost connection before proxy tunnel creation",
      {
        tunnelId,
        port,
        remoteAddress: tcpSocket?.remoteAddress ?? null,
  ...
  try {
    ...
    if (controlRemainingMs <= 0) {
      throw Object.assign(
        new Error("Tunnel setup timeout before CLI proxy connect"),
        { code: "ETIMEOUT" },
      );
    }
      ...
      if (!tcpSocket.destroyed) {
        ...
        try {
          tcpSocket.write(bytes, undefined, () => {
            ...
            latest.pendingWriteCount = Math.max(
              0,
              latest.pendingWriteCount - 1,
            );
            maybeEndLocalSocketAfterFlush();
          });
        } catch {
          current.pendingWriteCount = Math.max(
            0,
            current.pendingWriteCount - 1,
          );
          throw new Error(
            "failed to write proxy response bytes to localhost socket",
          );
        }
      }
    };
